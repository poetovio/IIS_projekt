...
DATA_DIRECTORY = PROJECT_ROOT / "data" / "processed" / "electricity" / "prices"
MODEL_DIRECTORY = PROJECT_ROOT / "models" / "classification"
...
def create_target(df, low_threshold, high_threshold):
...
def train_classifier():
    file_path = DATA_DIRECTORY / f"{ZONE}.csv"

    if not file_path.exists():
        raise FileNotFoundError(
            f"Energy data file not found: {file_path}"
        )
    ...
    low_threshold = train_df["price"].quantile(1 / 3)
    high_threshold = train_df["price"].quantile(2 / 3)
    ...
    mlflow.set_experiment("Energy_Price_Classification")

    with mlflow.start_run(run_name=f"classification_{ZONE}"):
        ...
        scaler_path = MODEL_DIRECTORY / f"{ZONE}_scaler.pkl"
